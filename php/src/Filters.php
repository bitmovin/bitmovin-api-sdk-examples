...
try {
    ...
    $encoding = createEncoding($exampleName, "Encoding with multiple filters applied to the video stream");
    ...
    $videoStream = createStream($encoding, $input, $inputFilePath, $h264Config);
    $audioStream = createStream($encoding, $input, $inputFilePath, $aacConfig);

    $watermarkFilter = createWatermarkFilter();
    $textFilter = createTextFilter();
    $deinterlaceFilter = createDeinterlaceFilter();

    createStreamFilters($encoding, $videoStream, [$watermarkFilter, $textFilter, $deinterlaceFilter]);

    createMp4Muxing($encoding, $output, '', [$videoStream, $audioStream], 'filters_applied.mp4');
...
function createWatermarkFilter()
{
    ...
    $watermarkFilter->image($configProvider->getWatermarkImagePath());
    $watermarkFilter->top(10);
    $watermarkFilter->left(10);
...
function createTextFilter()
{
    ...
    $textFilter->text($configProvider->getTextFilterText());
    $textFilter->x('main_w / 2');
    $textFilter->y('main_h / 2');
 ...
 * @throws BitmovinApiException
 */
function createDeinterlaceFilter()
{
    global $bitmovinApi;
 ...
 * applied.
 *
 * @param Encoding $encoding The encoding to which the video stream belongs to
 * @param Stream $stream The video stream to apply the filters to
 * @param Filter[] $filters A list of filter resources that have been created previously
 * @return StreamFilterList
 * @throws BitmovinApiException
 */
function createStreamFilters(Encoding $encoding, Stream $stream, array $filters)
{
    global $bitmovinApi;
    ...
    for ($i = 0; $i < sizeof($filters); $i++) {
        ...
        $streamFilter->id($filters[$i]->id);
        $streamFilter->position($i);

        $streamFilters[$i] = $streamFilter;
    }
