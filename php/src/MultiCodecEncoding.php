...
try {
    ...
    $h265Config = createH265Config();
    $h265Stream = createStream($encoding, $input, $inputFilePath, $h265Config);
    ...
    $ac3Stream = createStream($encoding, $input, $inputFilePath, $ac3Config);

    // Create an MP4 muxing with the H.264 and AAC streams
    createMp4Muxing($encoding, $output, 'mp4-h264-aac', [$h264Stream, $aacStream], 'video.mp4');

    // Create an MP4 muxing with the H.265 and AC3 streams
    createMp4Muxing($encoding, $output, 'mp4-h265-ac3', [$h265Stream, $ac3Stream], 'video.mp4');

    // Create a progressive TS muxing with the H.264 and AAC streams
    createProgressiveTsMuxing($encoding, $output, 'progressivets-h264-aac', [$h264Stream, $aacStream], 'video.ts');
 ...
 * @throws Exception
 */
function createProgressiveTsMuxing(Encoding $encoding, Output $output, string $outputPath, array $streams, string $filename)
{
    global $bitmovinApi;
 ...
 * @throws BitmovinApiException
 */
function createH265Config()
{
    global $bitmovinApi;

    $h265Config = new H265VideoConfiguration();
    $h265Config->name("H.265 1080p 1.5 Mbit/s");
    $h265Config->presetConfiguration(PresetConfiguration::VOD_STANDARD());
    $h265Config->height(1080);
    $h265Config->bitrate(1500000);

    return $bitmovinApi->encoding->configurations->video->h265->create($h265Config);
}
...
function createAc3Config()
{
    ...
    return $bitmovinApi->encoding->configurations->audio->ac3->create($ac3Config);
}
