...
/**
 * This example demonstrates the simplest mechanism to include a stereo audio stream in an MP4
 * output given the input file contains a stereo audio stream (and a video stream), with the use of
 * a single IngestInputStream.
 *
...
$exampleName = 'SimpleHandling_ImplicitMapping';
...
try {
    ...
    $encoding = createEncoding(
        "Audio Mapping - Simple Handling - Implicit Handling (Baseline)",
    ...
    $ingestInputStream = createIngestInputStream($encoding, $input, $inputFilePath);

    $videoStream = createStream($encoding, $ingestInputStream, $h264Config);
    $audioStream = createStream($encoding, $ingestInputStream, $aacConfig);

    createMp4Muxing($encoding, $output, "/", array($videoStream, $audioStream), "stereo-track.mp4");
