 ...
 *       e.g.: my-storage.biz
 *   <li>HTTP_INPUT_FILE_PATH - the path to a file containing a video stream
 *   <li>HTTP_INPUT_FILE_PATH_STEREO_SOUND - the path to an audio-only file containing a stereo
 *       stream
 *   <li>HTTP_INPUT_FILE_PATH_SURROUND_SOUND - the path to an audio-only file containing a 5.1
 *       stream
...
$exampleName = 'SimpleHandling_DistinctInputFiles';
...
try {
    ...
    $encoding = createEncoding(
        "Audio Mapping - Simple Handling - Distinct Input Files",
    ...
    $videoInputFilePath = $configProvider->getHttpInputFilePath();
    $stereoInputFilePath = $configProvider->getHttpInputFilePathWithStereoSound();
    $surroundInputFilePath = $configProvider->getHttpInputFilePathWithSurroundSound();
    ...
    $videoIngestInputStream = createIngestInputStream($encoding, $input, $videoInputFilePath);
    $stereoIngestInputStream = createIngestInputStream($encoding, $input, $stereoInputFilePath);
    $surroundIngestInputStream = createIngestInputStream($encoding, $input, $surroundInputFilePath);

    $videoStream = createStream($encoding, $videoIngestInputStream, $h264Config);
    $audioStream1 = createStream($encoding, $stereoIngestInputStream, $aacConfig);
    $audioStream2 = createStream($encoding, $surroundIngestInputStream, $ddConfig);

    createMp4Muxing($encoding, $output, "/", array($videoStream, $audioStream1, $audioStream2), "stereo-and-surround-tracks.mp4");
...
function createDdSurroundAudioConfig(): DolbyDigitalAudioConfiguration
{
    ...
    $config->name("Dolby Digital Channel Layout 5.1");
