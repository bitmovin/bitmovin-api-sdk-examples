...
$exampleName = 'StreamMerging_BackgroundAudio';
...
try {
    ...
    $inputFilePath = $configProvider->getHttpInputFilePathWithTwoStereoTracks();
    ...
    $mainAudioIngestInputStream =
        createIngestInputStreamForAudioTrack($encoding, $input, $inputFilePath, 0);
    $secondaryAudioIngestInputStream =
        createIngestInputStreamForAudioTrack($encoding, $input, $inputFilePath, 1);

    $secondaryAudioMixInputStream = new AudioMixInputStream();
    $secondaryAudioMixInputStream->channelLayout(AudioMixInputChannelLayout::CL_STEREO());

    for ($i = 0; $i <= 1; $i++) {
        $sourceChannel = new AudioMixInputStreamSourceChannel();
        $sourceChannel->type(AudioMixSourceChannelType::CHANNEL_NUMBER());
        $sourceChannel->channelNumber($i);
        $sourceChannel->gain(0.5);

        $inputStreamChannel = new AudioMixInputStreamChannel();
        $inputStreamChannel->inputStreamId($secondaryAudioIngestInputStream->id);
        $inputStreamChannel->outputChannelType(AudioMixChannelType::CHANNEL_NUMBER());
        $inputStreamChannel->outputChannelNumber($i);
        $inputStreamChannel->sourceChannels([$sourceChannel]);

        $secondaryAudioMixInputStream->audioMixChannels[] = $inputStreamChannel;
    }

    $secondaryAudioMixInputStream = $bitmovinApi->encoding->encodings->inputStreams->audioMix->create($encoding->id, $secondaryAudioMixInputStream);

    $videoStream = createStream($encoding, array($videoIngestInputStream), $h264Config);
    $audioStream = createStream($encoding, array($mainAudioIngestInputStream, $secondaryAudioMixInputStream), $aacConfig);

    createMp4Muxing($encoding, $output, "/", array($videoStream, $audioStream), "stereo-track-merged.mp4");
 ...
 * @param array $inputStreams The list of inputStream resources providing the input file, to be merged
 * @param CodecConfiguration $codecConfiguration The codec configuration to be applied to the stream
 * @return Stream
 * @throws BitmovinApiException
 */
function createStream(Encoding $encoding, array $inputStreams, CodecConfiguration $codecConfiguration): Stream
{
    global $bitmovinApi;
    ...
    for ($i = 0; $i < sizeof($inputStreams); $i++) {
        $streamInput = new StreamInput();
        $streamInput->inputStreamId($inputStreams[$i]->id);
