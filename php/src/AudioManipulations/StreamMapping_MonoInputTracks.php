...
use BitmovinApiSdk\Models\AudioMixChannelType;
use BitmovinApiSdk\Models\AudioMixInputChannelLayout;
use BitmovinApiSdk\Models\AudioMixInputStream;
use BitmovinApiSdk\Models\AudioMixInputStreamChannel;
use BitmovinApiSdk\Models\AudioMixInputStreamSourceChannel;
use BitmovinApiSdk\Models\AudioMixSourceChannelType;
...
$exampleName = 'StreamMapping_MonoInputTracks';
...
try {
    ...
    $inputFilePath = $configProvider->getHttpInputFilePathWithMultipleMonoAudioTracks();
    ...
    $h264Config = createH264VideoConfig();
    $aacConfig = createAacStereoAudioConfig();
    $ddConfig = createDdSurroundAudioConfig();

    $videoIngestInputStream = createIngestInputStream($encoding, $input, $inputFilePath);

    $stereoMap = [];
    $stereoMap[] = new ChannelMappingConfig(AudioMixChannelType::FRONT_LEFT(), 0);
    $stereoMap[] = new ChannelMappingConfig(AudioMixChannelType::FRONT_RIGHT(), 1);

    $surroundMap = [];
    $surroundMap[] = new ChannelMappingConfig(AudioMixChannelType::FRONT_LEFT(), 2);
    $surroundMap[] = new ChannelMappingConfig(AudioMixChannelType::FRONT_RIGHT(), 3);
    $surroundMap[] = new ChannelMappingConfig(AudioMixChannelType::BACK_LEFT(), 4);
    $surroundMap[] = new ChannelMappingConfig(AudioMixChannelType::BACK_RIGHT(), 5);
    $surroundMap[] = new ChannelMappingConfig(AudioMixChannelType::CENTER(), 6);
    $surroundMap[] = new ChannelMappingConfig(AudioMixChannelType::LOW_FREQUENCY(), 7);

    $stereoMixInputStream =
        createAudioMixInputStream(
            $encoding, $input, $inputFilePath, AudioMixInputChannelLayout::CL_STEREO(), $stereoMap);

    $surroundMixInputStream =
        createAudioMixInputStream(
            $encoding, $input, $inputFilePath, AudioMixInputChannelLayout::CL_5_1_BACK(), $surroundMap);

    $videoStream = createStream($encoding, $videoIngestInputStream, $h264Config);
    $audioStream1 = createStream($encoding, $stereoMixInputStream, $aacConfig);
    $audioStream2 = createStream($encoding, $surroundMixInputStream, $aacConfig);

    createMp4Muxing($encoding, $output, "/", array($videoStream, $audioStream1, $audioStream2), "stereo-and-surround-tracks-mapped.mp4");
...
class ChannelMappingConfig
{
    ...
    public $sourceChannelNumber;

    function __construct(AudioMixChannelType $outputChannelType, int $sourceChannelNumber)
    {
 ...
 * @throws BitmovinApiException
 */
function createH264VideoConfig(): H264VideoConfiguration
{
    global $bitmovinApi;
 ...
 * @throws BitmovinApiException
 */
function createAacStereoAudioConfig(): AacAudioConfiguration
{
    global $bitmovinApi;
 ...
 * @throws BitmovinApiException
 */
function createDdSurroundAudioConfig(): DolbyDigitalAudioConfiguration
{
    global $bitmovinApi;

    $config = new DolbyDigitalAudioConfiguration();
    $config -> name("DolbyDigital Channel Layout 5.1");
    $config -> bitrate(256000);
    $config -> channelLayout(DolbyDigitalChannelLayout::CL_5_1());
 ...
 * @param InputStream $inputStream The inputStream resource providing the input file
 * @param CodecConfiguration $codecConfiguration The codec configuration to be applied to the stream
 * @return Stream
 * @throws BitmovinApiException
 */
function createStream(Encoding $encoding, InputStream $inputStream, CodecConfiguration $codecConfiguration): Stream
{
    global $bitmovinApi;

    $streamInput = new StreamInput();
    $streamInput->inputStreamId($inputStream->id);

    $stream = new Stream();
    $stream->inputStreams([$streamInput]);
    $stream->codecConfigId($codecConfiguration->id);
    $stream->mode(StreamMode::STANDARD());
 ...
 * @param Input $input The input resource providing the input file
 * @param string $inputPath The path to the input file
 * @param int $position The relative position of the audio track to select in the input file
 * @return IngestInputStream
 * @throws BitmovinApiException
 */
function createIngestInputStreamForAudioTrack(Encoding $encoding, Input $input, string $inputPath, int $position): IngestInputStream
{
    global $bitmovinApi;
    ...
    $ingestInputStream->inputId($input->id);
    $ingestInputStream->inputPath($inputPath);
    $ingestInputStream->selectionMode(StreamSelectionMode::AUDIO_RELATIVE());
    $ingestInputStream->position($position);
 ...
 * @param AudioMixInputChannelLayout $channelLayout The configuration of the channel layout
 * @param array $mappingConfigs The configuration of the mapping of source channels to output channels
 * @return AudioMixInputStream
 * @throws BitmovinApiException
 */
function createAudioMixInputStream(Encoding $encoding, Input $input, string $inputFilePath, AudioMixInputChannelLayout $channelLayout, array $mappingConfigs): AudioMixInputStream
{
    global $bitmovinApi;

    $audioMixInputStream = new AudioMixInputStream();
    $audioMixInputStream->channelLayout($channelLayout);

    foreach ($mappingConfigs as $mappingConfig) {
        $audioIngestInputStream =
            createIngestInputStreamForAudioTrack(
                $encoding, $input, $inputFilePath, $mappingConfig -> sourceChannelNumber);

        $outputChannel = new AudioMixInputStreamChannel();
        $outputChannel->inputStreamId($audioIngestInputStream->id);
        $outputChannel->outputChannelType($mappingConfig->outputChannelType);

        $sourceChannel = new AudioMixInputStreamSourceChannel();
        $sourceChannel->type(AudioMixSourceChannelType::CHANNEL_NUMBER());
        $sourceChannel->channelNumber(0);
        $outputChannel->sourceChannels([$sourceChannel]);

        $audioMixInputStream->audioMixChannels[] = $outputChannel;
    }
