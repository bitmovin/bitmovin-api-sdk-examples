...
$exampleName = 'ChannelMixing_Downmixing';
...
try {
    ...
    $encoding = createEncoding(
        "Audio Mapping - Channel Mixing - Downmixing",
        "Input with 5.1 track -> Output with downmixed stereo track");

    $input = createHttpInput($configProvider->getHttpInputHost());
    $inputFilePath = $configProvider->getHttpInputFilePathWithSurroundSound();
    ...
    $audioIngestInputStream = createIngestInputStream($encoding, $input, $inputFilePath);

    $channelConfigLeft = new DownmixConfig(AudioMixChannelType::FRONT_LEFT());
    $channelConfigLeft->addSourceChannels(AudioMixSourceChannelType::FRONT_LEFT(), 1.0);
    $channelConfigLeft->addSourceChannels(AudioMixSourceChannelType::BACK_LEFT(), 0.8);
    $channelConfigLeft->addSourceChannels(AudioMixSourceChannelType::CENTER(), 0.5);

    $channelConfigRight = new DownmixConfig(AudioMixChannelType::FRONT_RIGHT());
    $channelConfigRight->addSourceChannels(AudioMixSourceChannelType::FRONT_RIGHT(), 1.0);
    $channelConfigRight->addSourceChannels(AudioMixSourceChannelType::BACK_RIGHT(), 0.8);
    $channelConfigRight->addSourceChannels(AudioMixSourceChannelType::CENTER(), 0.5);

    $audioMixInputStream = createDownmixInputStream($encoding, $audioIngestInputStream, array($channelConfigLeft, $channelConfigRight));

    $videoStream = createStream($encoding, $videoIngestInputStream, $h264Config);
    $audioStream = createStream($encoding, $audioMixInputStream, $aacConfig);

    createMp4Muxing($encoding, $output, "/", array($videoStream, $audioStream), "stereo-track-downmixed.mp4");
...
class DownmixConfig
{
    ...
    function __construct($outputChannelType)
    {
        $this->outputChannelType = $outputChannelType;
        $this->sourceChannels = [];
    }

    function addSourceChannels($sourceChannelType, $gain)
    {
        $mappingSource = new DownmixConfigChannel($sourceChannelType, $gain);
        $this->sourceChannels[] = $mappingSource;
    }
}

class DownmixConfigChannel
{
    public $sourceChannelType;
    public $gain;

    function __construct($sourceChannelType, $gain)
    {
 ...
 * @param array $channelConfigs The configuration of the source channel mixing and mapping to the output
 *     channels
 * @return AudioMixInputStream
 * @throws BitmovinApiException
 */
function createDownmixInputStream(Encoding $encoding, InputStream $inputStream, array $channelConfigs): AudioMixInputStream
{
    global $bitmovinApi;

    $audioMixInputStream = new AudioMixInputStream();
    $audioMixInputStream->name = "Downmixing 5.1 to stereo";
    $audioMixInputStream->channelLayout = AudioMixInputChannelLayout::CL_STEREO();

    foreach ($channelConfigs as $channelConfig) {
        $outputChannel = new AudioMixInputStreamChannel();
        $outputChannel->inputStreamId($inputStream->id);
        $outputChannel->outputChannelType($channelConfig->outputChannelType);

        foreach ($channelConfig->sourceChannels as $channelSource) {
            $sourceChannel = new AudioMixInputStreamSourceChannel();
            $sourceChannel->type($channelSource->sourceChannelType);
            $sourceChannel->gain($channelSource->gain);
