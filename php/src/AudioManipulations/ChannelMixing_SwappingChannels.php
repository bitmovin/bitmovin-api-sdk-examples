...
$exampleName = 'ChannelMixing_SwappingChannels';
...
try {
    ...
    $inputFilePath = $configProvider->getHttpInputFilePathWithStereoSound();
    ...
    $audioMixInputStream->name("Swapping channels 0 and 1");
    $audioMixInputStream->channelLayout(AudioMixInputChannelLayout::CL_STEREO());

    $sourceChannel0 = new AudioMixInputStreamSourceChannel();
    $sourceChannel0->type(AudioMixSourceChannelType::CHANNEL_NUMBER());
    $sourceChannel0->channelNumber(0);

    $sourceChannel1 = new AudioMixInputStreamSourceChannel();
    $sourceChannel1->type(AudioMixSourceChannelType::CHANNEL_NUMBER());
    $sourceChannel1->channelNumber(1);

    $outputChannel0 = new AudioMixInputStreamChannel();
    $outputChannel0->inputStreamId($audioIngestInputStream->id);
    $outputChannel0->outputChannelType(AudioMixChannelType::CHANNEL_NUMBER());
    $outputChannel0->outputChannelNumber(0);
    $outputChannel0->sourceChannels[] = $sourceChannel1;

    $outputChannel1 = new AudioMixInputStreamChannel();
    $outputChannel1->inputStreamId($audioIngestInputStream->id);
    $outputChannel1->outputChannelType(AudioMixChannelType::CHANNEL_NUMBER());
    $outputChannel1->outputChannelNumber(1);
    $outputChannel1->sourceChannels[] = $sourceChannel0;

    $audioMixInputStream->audioMixChannels(array($outputChannel0, $outputChannel1));
    $audioMixInputStream = $bitmovinApi->encoding->encodings->inputStreams->audioMix->create($encoding->id, $audioMixInputStream);
    ...
    createMp4Muxing($encoding, $output, "/", array($videoStream, $audioStream), "stereo-track-swapped.mp4");
