...
use BitmovinApiSdk\Models\BroadcastTsAudioInputStreamConfiguration;
use BitmovinApiSdk\Models\BroadcastTsMuxing;
use BitmovinApiSdk\Models\BroadcastTsMuxingConfiguration;
use BitmovinApiSdk\Models\BroadcastTsVideoInputStreamConfiguration;
...
$exampleName = 'MultiLanguageBroadcastTs';
...
try {
    ...
    $encoding = createEncoding($exampleName, "Encoding with different codecs and muxing types");
    ...
    $videoConfig = createH264Config();
    $videoStream = createStream($encoding, $input, $inputFilePath, $videoConfig, StreamSelectionMode::VIDEO_RELATIVE(), 0);

    $audioConfiguration = createMp2AudioConfig();

    $audioStreams = [
        "eng" => createStream($encoding, $input, $inputFilePath, $audioConfiguration, StreamSelectionMode::AUDIO_RELATIVE(), 0),
        "deu" => createStream($encoding, $input, $inputFilePath, $audioConfiguration, StreamSelectionMode::AUDIO_RELATIVE(), 1)
    ];

    createBroadcastTsMuxing($encoding, $output, '', $videoStream, $audioStreams);
 ...
 *
 * @param Encoding $encoding The encoding to which the muxing will be added
 * @param Output $output The output resource to which the unencrypted segments will be written to
 * @param string $outputPath The output path where the unencrypted segments will be written to
 * @param Stream $videoStream The video stream to be included in the muxing
 * @param Stream[] $audioStreams A map of audio streams to be included in the muxing, with the key value
 *     specifying their language tag
 * @return BroadcastTsMuxing
 * @throws BitmovinApiException
 * @throws Exception
 */
function createBroadcastTsMuxing(Encoding $encoding, Output $output, string $outputPath, Stream $videoStream, array $audioStreams)
{
    global $bitmovinApi;

    $muxing = new BroadcastTsMuxing();
    $muxing->filename("broadcast.ts");
    $muxing->name("BroadcastTS muxing");
    $muxing->segmentLength(4.0);
    $muxing->outputs([buildEncodingOutput($output, $outputPath)]);
    $muxing->configuration(new BroadcastTsMuxingConfiguration());

    $videoInputStreamConfiguration = new BroadcastTsVideoInputStreamConfiguration();
    $videoInputStreamConfiguration->streamId($videoStream->id);

    $muxing->configuration->videoStreams[] = $videoInputStreamConfiguration;

    $muxing->streams[] = buildMuxingStream($videoStream);

    $pid = 2000;

    foreach ($audioStreams as $lang => $audioStream) {
        $audioInputStreamConfiguration = new BroadcastTsAudioInputStreamConfiguration();
        $audioInputStreamConfiguration->streamId($audioStream->id);
        $audioInputStreamConfiguration->packetIdentifier($pid++);
        $audioInputStreamConfiguration->language($lang);

        $muxing->configuration->audioStreams[] = $audioInputStreamConfiguration;
        $muxing->streams[] = buildMuxingStream($audioStream);
    }

    return $bitmovinApi->encoding->encodings->muxings->broadcastTs->create($encoding->id, $muxing);
}

function buildMuxingStream(Stream $stream)
{
 ...
 * @throws BitmovinApiException
 */
function createMp2AudioConfig()
{
    global $bitmovinApi;

    $config = new Mp2AudioConfiguration();
    $config->name("MP2 96 kbit/s");
 ...
 * @param StreamSelectionMode $streamSelectionMode Specifies the strategy how the stream will be selected from an input
 *     file with multiple streams
 * @param int $position Depending on streamSelectionMode this specifies the absolute or relative
 *     position of the stream to be selected
 * @return Stream
 * @throws BitmovinApiException
 */
function createStream(Encoding $encoding, Input $input, string $inputPath, CodecConfiguration $codecConfiguration, StreamSelectionMode $streamSelectionMode, int $position)
{
    global $bitmovinApi;
    ...
    $streamInput->selectionMode($streamSelectionMode);
