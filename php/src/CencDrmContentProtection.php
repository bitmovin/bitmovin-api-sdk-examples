...
use BitmovinApiSdk\Models\CencFairPlay;
use BitmovinApiSdk\Models\CencWidevine;
...
$exampleName = 'CencDrmContentProtection';
...
try {
    ...
    $encoding = createEncoding($exampleName, "Encoding with CENC DRM content protection");
    ...
    $videoMuxing = createFmp4Muxing($encoding, $h264Stream);
    $audioMuxing = createFmp4Muxing($encoding, $aacStream);
 ...
 * @throws Exception
 */
function createDrmConfig(Encoding $encoding, Muxing $muxing, Output $output, string $outputPath): CencDrm
{
    global $bitmovinApi, $configProvider;

    $widevineDrm = new CencWidevine();
    $widevineDrm->pssh($configProvider->getDrmWidevinePssh());
 ...
 * @throws Exception
 */
function createFmp4Muxing(Encoding $encoding, Stream $stream): Fmp4Muxing
{
    global $bitmovinApi;
