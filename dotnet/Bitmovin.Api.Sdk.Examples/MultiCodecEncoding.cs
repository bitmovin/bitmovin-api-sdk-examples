...
{
    /// <summary>
    /// This example demonstrates how to use different codecs and muxing types in a single encoding.<para />
    ///
    ...
    public class MultiCodecEncoding : IExample
    {
        ...
        /// </summary>
        private Task<H265VideoConfiguration> CreateH265VideoConfiguration()
        {
        ...
        /// <param name="streams">A list of streams to be added to the muxing</param>
        /// <param name="fileName">The name of the file that will be written to the output</param>
        private Task CreateProgressiveTsMuxing(Models.Encoding encoding, Output output,
            string outputPath, List<Stream> streams, string fileName)
        {
            var muxing = new ProgressiveTsMuxing()
            {
                Outputs = new List<EncodingOutput>() {BuildEncodingOutput(output, outputPath)},
                Filename = fileName
            };

            streams.ForEach(stream => muxing.Streams.Add(new MuxingStream() {StreamId = stream.Id}));

            return _bitmovinApi.Encoding.Encodings.Muxings.ProgressiveTs.CreateAsync(encoding.Id, muxing);
        }
        ...
        {
            return Path.Join(_configProvider.GetS3OutputBasePath(), ClassName, relativePath);
        }
