...
const exampleName = 'FixedBitrateLadder';
...
async function main() {
  const encoding = await createEncoding(exampleName, 'Encoding with multiple MP4 muxings');
  ...
  for (let videoConfig of videoConfigurations) {
    ...
    await createMp4Muxing(encoding, output, '/', [videoStream, audioStream], `video_${videoConfig.height}.mp4`);
  }
