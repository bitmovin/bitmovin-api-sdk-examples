...
const exampleName = 'SimpleHandlingImplicitMapping';
...
async function main() {
  ...
  const ingestInputStream = await createIngestInputStream(encoding, input, inputFilePath);

  const videoStream = await createStream(encoding, ingestInputStream, h264Config);
  const audioStream = await createStream(encoding, ingestInputStream, aacConfig);
