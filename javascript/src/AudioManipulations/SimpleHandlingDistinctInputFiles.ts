...
const exampleName = 'SimpleHandlingDistinctInputFiles';
...
async function main() {
  ...
  const videoInputFilePath = configProvider.getHttpInputFilePath();
  const stereoInputFilePath = configProvider.getHttpInputFilePathWithStereoSound();
  const surroundInputFilePath = configProvider.getHttpInputFilePathWithSurroundSound();

  const videoIngestInputStream = await createIngestInputStream(encoding, input, videoInputFilePath);
  const stereoIngestInputStream = await createIngestInputStream(encoding, input, stereoInputFilePath);
  const surroundIngestInputStream = await createIngestInputStream(encoding, input, surroundInputFilePath);

  const videoStream = await createStream(encoding, videoIngestInputStream, h264Config);
  const audioStream1 = await createStream(encoding, stereoIngestInputStream, aacConfig);
  const audioStream2 = await createStream(encoding, surroundIngestInputStream, ddConfig);
