...
const exampleName = 'ChannelMixingDownmixing';
...
class DownmixConfig {
  ...
  sourceChannels: DownmixConfigChannel[];

  constructor(outputChannelType: AudioMixChannelType) {
    this.outputChannelType = outputChannelType;
    this.sourceChannels = [];
  }

  addSourceChannel(sourceChannelType: AudioMixSourceChannelType, gain: number) {
    const mappingSource = new DownmixConfigChannel(sourceChannelType, gain);
    this.sourceChannels.push(mappingSource);
  }
}

class DownmixConfigChannel {
  sourceChannelType: AudioMixSourceChannelType;
  gain: number;

  constructor(sourceChannelType: AudioMixSourceChannelType, gain: number) {
...
async function main() {
  ...
  const inputFilePath = configProvider.getHttpInputFilePathWithSurroundSound();
  ...
  const channelConfigLeft = new DownmixConfig(AudioMixChannelType.FRONT_LEFT);
  ...
  const channelConfigRight = new DownmixConfig(AudioMixChannelType.FRONT_RIGHT);
  ...
  const audioMixInputStream = await createDownmixInputStream(encoding, audioIngestInputStream, [
    channelConfigLeft,
    channelConfigRight,
  ]);
  ...
  await createMp4Muxing(encoding, output, '/', [videoStream, audioStream], 'stereo-track-downmixed.mp4');
 ...
 *     channels
 */
function createDownmixInputStream(
  encoding: Encoding,
  inputStream: InputStream,
  channelConfigs: DownmixConfig[]
): Promise<AudioMixInputStream> {
  const audioMixInputStream = new AudioMixInputStream({
  ...
  for (const channelConfig of channelConfigs) {
    ...
    for (const channelSource of channelConfig.sourceChannels) {
