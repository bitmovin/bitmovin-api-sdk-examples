...
public class RedundantRtmpLiveEncoding {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Encoding encoding =
        createEncoding(
            "Redundant RTMP Live Encoding Test",
    ...
    // RedundantRtmpInput input = getRedundantRtmpInput(); // Alternative: get an automatically
    // generated  Redundant RTMP Input
  ...
  /**
   * Retrieves the first Redundant RTMP input. This is an automatically generated resource and
   * read-only.
   *
   * <p>API endpoint:
   ...
   *
   * <p>Note that this will result in two RTMP Push URLs which you can use to ingest your primary
   * and backup streams. An RTMP Push URL has the format:
   *
   * <p>rtmp://server_ip/applicationName/streamKey
   *
   * <p>Note that the server_ip (Server's IP address) will be determined by the encoding after it
   * has been started, and is then available via the API. This script, as well as the Bitmovin
   * Dashboard, will provide the details when they are available.
   *
   * <p>The RTMP Push URLs are: rtmp://server_ip/live/streamKeyPrimary (for the primary stream)
   * rtmp://server_ip/live-backup/streamKeyBackup (for the backup stream)
   *
   * <p>streamKeyPrimary and streamKeyBackup are set in the initialization section of the class.
   *
   * <p>Once you have the primary and backup RTMP Push URLs, please consult the manuals of your
   * contributing encooder (e.g. ffmpeg, OBS, Elemental, Teradek, Teracue or any other vendor) how
   * to push a live stream to an RTMP Push URL.
   *
   * <p>When sending RTMP Push streams from your contributing encoders, please make sure that they
   * have the same content but run independently, so that anything that may affect the primary
   * stream does not affect the backup. Also please be aware that the encoding always processes the
   * stream that comes first. Thus, the primary stream should be applied first, then the backup. If
   * the primary fails later, the backup will kick in and run as long as it is up, even if the
   * primary is restored.
   *
   * <p>API endpoint:
  ...
  private static RedundantRtmpInput createRedundantRtmpInput() throws BitmovinException {
    ...
    List<RtmpIngestPoint> rtmpIngestPointList =
        Arrays.asList(rtmpIngestPoint, rtmpIngestPointBackup);
