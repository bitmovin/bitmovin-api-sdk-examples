...
import com.bitmovin.api.sdk.model.DeinterlaceFilter;
...
import com.bitmovin.api.sdk.model.TextFilter;
...
public class Filters {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Encoding encoding =
        createEncoding(
            "Filter example", "Encoding with multiple filters applied to the video stream");
    ...
    createMp4Muxing(
        encoding, output, "/", Arrays.asList(videoStream, audioStream), "filters_applied.mp4");
  ...
  private static StreamFilterList createStreamFilterList(
      Encoding encoding, Stream stream, List<Filter> filters) {
    int position = 0;
  ...
  private static TextFilter createTextFilter() throws BitmovinException {
    ...
    textFilter.setText(configProvider.getTextFilterText());
    textFilter.setX("main_w / 2");
    textFilter.setY("main_h / 2");

    return bitmovinApi.encoding.filters.text.create(textFilter);
  }

  /**
   * Creates a deinterlace filter, which can be used to convert <a
   * href="https://en.wikipedia.org/wiki/Interlaced_video">interlaced video</a> to <a
   * href="https://en.wikipedia.org/wiki/Progressive_scan">progressive video</a>. A deinterlace
   * filter has no affect on progressive files. HLS <a
  ...
  public static String buildAbsolutePath(String relativePath) {
    String className = Filters.class.getSimpleName();
