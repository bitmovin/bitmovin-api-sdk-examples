...
EXAMPLE_NAME = "ChannelMixingSwappingChannels-{}".format(datetime.now().isoformat(timespec='seconds'))
...
def main():
    encoding = _create_encoding(
        name="Audio Mapping - Channel Mixing - Swapping Channels",
        description="Input with stereo track -> Output with reversed stereo track"
    )
    ...
    input_file_path = config_provider.get_http_input_file_path_with_stereo_sound()
    ...
    source_channel0 = AudioMixInputStreamSourceChannel(type_=AudioMixSourceChannelType.CHANNEL_NUMBER, channel_number=0)

    source_channel1 = AudioMixInputStreamSourceChannel(type_=AudioMixSourceChannelType.CHANNEL_NUMBER, channel_number=1)

    output_channel0 = AudioMixInputStreamChannel(
        ...
        source_channels=[source_channel1]
    )

    output_channel1 = AudioMixInputStreamChannel(
        input_stream_id=audio_ingest_input_stream.id,
        output_channel_type=AudioMixChannelType.CHANNEL_NUMBER,
        output_channel_number=1,
        source_channels=[source_channel0]
    )

    audio_mix_input_stream.audio_mix_channels = [output_channel0, output_channel1]
    audio_mix_input_stream = bitmovin_api.encoding.encodings.input_streams.audio_mix.create(
    ...
    _create_mp4_muxing(
        ...
        file_name="stereo-track-swapped.mp4"
    )
