...
from bitmovin_api_sdk import AacAudioConfiguration, AclEntry, AclPermission, BitmovinApi, BitmovinApiLogger, \
     CodecConfiguration, Encoding, EncodingOutput, H264VideoConfiguration, HttpInput, IngestInputStream, Input, \
     InputStream, MessageType, Mp4Muxing, MuxingStream, Output, PresetConfiguration, S3Output, Status, Stream, \
     StreamInput, StreamMode, StreamSelectionMode, Task
...
EXAMPLE_NAME = "SimpleHandlingImplicitMapping-{}".format(datetime.now().isoformat(timespec='seconds'))
...
def main():
    encoding = _create_encoding(
        name="Audio Mapping - Simple Handling - Implicit Handling (Baseline)",
        description="Input with stereo track -> Output with stereo track"
    )
    ...
    ingest_input_stream = _create_ingest_input_stream(
        encoding=encoding,
        input=http_input,
    ...
    video_stream = _create_stream(
        encoding=encoding,
        input_stream=ingest_input_stream,
