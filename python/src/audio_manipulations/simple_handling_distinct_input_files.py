...
from bitmovin_api_sdk import AacAudioConfiguration, AclEntry, AclPermission, BitmovinApi, BitmovinApiLogger, \
    CodecConfiguration, DolbyDigitalAudioConfiguration, DolbyDigitalChannelLayout, Encoding, EncodingOutput, \
...
EXAMPLE_NAME = "SimpleHandlingDistinctInputFiles-{}".format(datetime.now().isoformat(timespec='seconds'))
...
def main():
    encoding = _create_encoding(
        name="Audio Mapping - Simple Handling - Distinct Input Files",
        description="Separate inputs for video, stereo and surround tracks -> Output with 2 audio tracks"
    )
    ...
    video_input_file_path = config_provider.get_http_input_file_path()
    stereo_input_file_path = config_provider.get_http_input_file_path_with_stereo_sound()
    surround_input_file_path = config_provider.get_http_input_file_path_with_surround_sound()

    video_ingest_input_stream = _create_ingest_input_stream(
        encoding=encoding,
        input=http_input,
        input_path=video_input_file_path
    )
    stereo_ingest_input_stream = _create_ingest_input_stream(
        encoding=encoding,
        input=http_input,
        input_path=stereo_input_file_path
    )
    surround_ingest_input_stream = _create_ingest_input_stream(
        encoding=encoding,
        input=http_input,
        input_path=surround_input_file_path
    )
    ...
    audio_stream1 = _create_stream(
        encoding=encoding,
        input_stream=stereo_ingest_input_stream,
        codec_configuration=aac_config
    )
    audio_stream2 = _create_stream(
        encoding=encoding,
        input_stream=surround_ingest_input_stream,
    ...
    _create_mp4_muxing(
        ...
        file_name="stereo-and-surround-tracks.mp4"
    )
