 ...
 *   <li>properties file located in the root folder of the GO examples at ./examples.properties
...
func createStream(encoding model.Encoding, input model.Input, inputPath string, codecConfiguration model.CodecConfiguration) (*model.Stream, error) {
	baseInput, ok := input.(model.S3Input)
	if !ok {
		return nil, fmt.Errorf("unrecognized input type")
	}
	baseCodecConfig, ok := codecConfiguration.(model.BaseCodecConfiguration)
	if !ok {
		return nil, fmt.Errorf("unrecognized codec configuration")
	}

	streamInput := model.StreamInput{
		InputId:       baseInput.Id,
		InputPath:     &inputPath,
	...
	stream := model.Stream{
		...
		CodecConfigId: baseCodecConfig.Id,
...
func buildEncodingOutput(output model.Output, outputPath string) (*model.EncodingOutput, error) {
	...
	if !ok {
		return nil, fmt.Errorf("unrecognized output type")
	}
...
func createDrmConfig(encoding model.Encoding, muxing model.Muxing, output model.Output, outputPath string) (*model.CencDrm, error) {
	...
	if !ok {
		return nil, fmt.Errorf("unrecognized muxing type")
	}
